...
def test_init(config):
    account = EthereumAccount(config.ethereum.accounts[0].as_dict)
    assert(account)
    assert(account.address)

    account = EthereumAccount(config.ethereum.accounts[0].as_tuple)
    assert(account)
    assert(account.address)

    account = EthereumAccount(config.ethereum.accounts[0].as_list)
    assert(account)
    assert(account.address)

    account = EthereumAccount(config.ethereum.accounts[0].address, 'test-password', 'test-keyvalue')
    assert(account)
    assert(account.address)

def test_is_address_equal(config):
    account = EthereumAccount(config.ethereum.accounts[0].as_dict)
    assert(account.is_address_equal(config.ethereum.accounts[0].test_address))
...
def test_is_password(config):
    account = EthereumAccount(config.ethereum.accounts[0].as_dict)
    assert(account.password == config.ethereum.accounts[0].test_password)

def test_is_key_value(config):
    account = EthereumAccount(config.ethereum.accounts[0].as_dict)
    assert(account.key_value == config.ethereum.accounts[0].test_key_value)

def test_address(config):
    account = EthereumAccount(config.ethereum.accounts[0].as_dict)
    assert(account.address.lower() == config.ethereum.accounts[0].test_address.lower())

def test_as_checksum_address(config):
    account = EthereumAccount(config.ethereum.accounts[0].as_dict)
    assert(account.as_checksum_address == Web3.toChecksumAddress(config.ethereum.accounts[0].test_address))

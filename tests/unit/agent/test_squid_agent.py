...
def _register_asset_and_listing(ocean, resources, config):
    account = ocean.load_account(config.accounts[0].as_dict)
    agent = SquidAgent(ocean)
    assert(agent)
    asset_data = RemoteDataAsset.create_with_url('test url asset', resources.asset_remote)
    assert(asset_data)
    listing = agent.register_asset_and_listing(asset_data, resources.listing_data, account)
    return (listing, agent, asset_data)

def _purchase_asset(ocean, resources, config):
    listing, agent, asset = _register_asset_and_listing(ocean, resources, config)
...
def test_register_asset_and_listing(ocean, resources, config):
    listing, agent, asset = _register_asset_and_listing(ocean, resources, config)
    assert(listing)
...
def test_register_bundle_asset(ocean, resources, config):
    ...
    agent = SquidAgent(ocean)
    assert(agent)
    asset_bundle = BundleAsset.create('test bundle asset')
    for index in range(0, 5):
        asset_data = RemoteDataAsset.create_with_url('test remote asset', resources.asset_remote)
        assert(asset_data)
        asset_bundle.add(f'name_{index}', asset_data)
    listing = agent.register_asset_and_listing(asset_bundle, resources.listing_data, account)
    assert(listing)
...
def test_register_asset_invalid_data(ocean, resources, config):
    ...
    agent = SquidAgent(ocean)
    assert(agent)
    asset_data = DataAsset.create('test remote asset', 'some data that is never going to be saved')
...
def test_validate_asset(ocean):
    agent = SquidAgent(ocean, TEST_INIT_PARMS)
    assert(agent)
    asset_data = RemoteDataAsset.create_with_url('TestAsset', 'http://dex.sg')
    assert(agent.validate_asset(asset_data))
...
def test_is_access_granted_for_asset(ocean, resources, config):
    ...
    assert(agent.is_access_granted_for_asset(asset.did, account, purchase.purchase_id, ))

def test_purchase_wait_for_completion(ocean, resources, config):
    purchase, listing, agent, asset, account = _purchase_asset(ocean, resources, config)
    assert(agent.purchase_wait_for_completion(asset.did, account, purchase.purchase_id, 30))
    # test raised error if purchase failed
    with pytest.raises(ValueError):
        agent.purchase_wait_for_completion(asset.did, account, None, 30)
...
def test_price_out_of_range(ocean, resources, config):
    ...
    agent = SquidAgent(ocean)
    assert(agent)
    asset_data = RemoteDataAsset.create_with_url('test squid asset with url', resources.asset_remote)
    assert(asset_data)

...
if [ -f artifacts/Provenance.development.json ]; then
    cp $CONTRACT_FOLDER/Provenance.development.json $CONTRACT_FOLDER/Provenance.spree.json
fi

if [ -f artifacts/DIDRegistry.development.json ]; then
    cp $CONTRACT_FOLDER/DIDRegistry.development.json $CONTRACT_FOLDER/DIDRegistry.spree.json
fi

rm $CONTRACT_FOLDER/*.mainnet.json
rm $CONTRACT_FOLDER/*.pacific.json
rm $CONTRACT_FOLDER/*.kovan.json
rm $CONTRACT_FOLDER/*.duero.json
rm $CONTRACT_FOLDER/*.nile.json
